...
public class FunctionFitness2 implements IFunctionFitness {
	private double bias = 0.0; // Para ajustar valores para o fitness maiores que zero
	private double f;
	
	public FunctionFitness2() {
	}
	...
	public double function(double[] x) {
		f = 55 - Math.pow(x[0] - 1, 2) - Math.pow(x[1] - 3, 2) - Math.pow(x[2] - 4, 2) - Math.pow(x[3] - 5, 2) - Math.pow(x[4] - 2, 2) ;
		return(f);
	}
